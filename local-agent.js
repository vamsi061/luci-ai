/**
 * Luci Playwright Local Agent Server
 * Run this locally on your machine to grant Luci real control over your browser!
 * 
 * Setup instructions:
    ...
    switch (type) {
      case "browserOpen": {
        ...
        if (!destination.startsWith("http://") && !destination.startsWith("https://")) {
          if (destination.toLowerCase().includes("maps") || destination.toLowerCase().includes("google maps")) {
            const mapQ = destination.replace(/^(google\s+)?maps\s+/gi, "").replace(/search|find|show/gi, "").trim();
            destination = mapQ ? `https://www.google.com/maps/search/${encodeURIComponent(mapQ)}` : "https://www.google.com/maps";
          } else if (destination.toLowerCase().includes("youtube") || destination.toLowerCase() === "youtube") {
      ...
      case "browserReadContent": {
        logAndBroadcast("Extracting text and structured elements from active page...", "info");
        ...
        return res.json({
          result: {
            ...
            paragraphs: paragraphs.filter(p => p && p.trim().length > 10).slice(0, 15)
          }
        });
      }

...
interface LuciCoreVisualizerProps {
  session: LuciAudioSession | null;
  state: LiveState;
  themeColor: string; // Violet, crimson, emerald, celestial, gold, rose, charcoal
  activeEmotion?: LuciEmotion;
  characterState: "idle" | "thinking" | "talking";
}

export const LuciCoreVisualizer: React.FC<LuciCoreVisualizerProps> = ({
  session,
  state,
  themeColor,
  activeEmotion = "idle",
  ...
  const handleVideoError = (videoName: string) => {
    console.warn(`[Luci Web Video] Failed to load video source for: ${videoName}`);
    setHasError(true);
  };
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <canvas
        id="luci-hologram-living-canvas"
        ref={canvasRef}

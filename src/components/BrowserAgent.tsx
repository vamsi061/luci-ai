...
export const BrowserAgent: React.FC<BrowserAgentProps> = ({
  ...
  const checkIsRestricted = (urlStr: string): { restricted: boolean; reason: string } => {
    ...
    try {
      ...
      if (hostname.includes("google.com") && !parsed.pathname.includes("/search") && !parsed.pathname.includes("/maps") && !hostname.includes("maps")) {
        return { 
          restricted: true, 
  ...
  useEffect(() => {
    const handleNavigationMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === "NAVIGATE" && event.data.url) {
        console.log("[Luci Browser] Same-origin child iframe navigated to:", event.data.url);
  ...
  useEffect(() => {
    ...
    const runVoiceAutomation = async () => {
      try {
        // Priority 1: Delegate to local Playwright agent if available
        if (isLocalConnected) {
          try {
            ...
            console.warn("[BrowserAgent] Local Playwright agent call error, falling back to in-browser execution:", e);
          }
        }
        ...
        switch (type) {
          ...
          case "browserSearch": {
            ...
            const isMapsQuery = lowerQ.includes("map") || lowerQ.includes("directions") || lowerQ.includes("location") || (activeTab && (activeTab.url.includes("google.com/maps") || activeTab.url.includes("maps.google.com")));
            const isYtQuery = !isMapsQuery && (lowerQ.includes("youtube") || lowerQ.includes("video") || (activeTab && activeTab.url.includes("youtube.com")));

            if (isMapsQuery) {
              const cleanMapQ = query.replace(/^(google\s+)?maps\s+/gi, "").replace(/search|find|show/gi, "").trim();
              const destUrl = `https://www.google.com/maps/search/${encodeURIComponent(cleanMapQ || query)}`;
              navigateToUrl(destUrl);
              callback({ result: `Searching Google Maps for "${cleanMapQ || query}".` });
            } else if (isYtQuery) {
              ...
              navigateToUrl(destUrl);
              callback({ result: `Searching YouTube for "${cleanYtQ || query}".` });
            } else {
              const searchUrl = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
              navigateToUrl(searchUrl);
              callback({ result: `Searching for "${query}".` });
            }
            break;
          }
          case "browserClick": {
            const selector = (args.selector || "").trim();
            if (!selector) throw new Error("Missing selector or item description for click.");

            // 1. YouTube video result selection logic
            if (ytSearchResults.length > 0 || (activeTab && activeTab.url.includes("youtube"))) {
              let targetVideo = null;
              if (selector.startsWith("video-")) {
                const vidId = selector.replace("video-", "");
                targetVideo = ytSearchResults.find(v => v.videoId === vidId) || { videoId: vidId, title: "Selected Video" };
              } else if (selector === "first" || selector === "top" || selector === "1" || selector.includes("first video") || selector === "play") {
                targetVideo = ytSearchResults[0];
              } else if (selector === "2" || selector.includes("second video")) {
                targetVideo = ytSearchResults[1];
              } else if (selector === "3" || selector.includes("third video")) {
                targetVideo = ytSearchResults[2];
              } else {
                targetVideo = ytSearchResults.find(v => v.title.toLowerCase().includes(selector.toLowerCase()));
              }

              if (targetVideo && targetVideo.videoId) {
                const videoUrl = `https://youtube.com/watch?v=${targetVideo.videoId}`;
                navigateToUrl(videoUrl);
                callback({ result: `Playing "${targetVideo.title || "video"}" on YouTube.` });
                break;
              }
            }

            // 2. In-iframe document DOM clicking
            const iframe = iframeRef.current;
            if (iframe && iframe.contentWindow) {
              try {
                ...
                if (!element) {
                  const candidates = Array.from(doc.querySelectorAll('a, button, [role="button"], input[type="submit"], h3, span, div')) as HTMLElement[];
                  element = candidates.find(el => el.textContent && el.textContent.toLowerCase().includes(selector.toLowerCase())) as HTMLElement;
                }

                if (element) {
                  element.click();
                  callback({ result: `Successfully clicked target item.` });
                  break;
                }
              } catch (e) {
            ...
            callback({ result: `Executed click action for target "${args.description || selector}".` });
            break;
          }
          case "browserMediaControl": {
            ...
            if (iframe && iframe.contentWindow) {
              try {
                ...
                if (video) {
                  ...
                  else if (action === "rewind") video.currentTime -= 30;
                  commandSent = true;
                }
              } catch (e) {}
              ...
              if (!commandSent) {
                ...
                else if (action === "mute") func = "mute";
                else if (action === "unmute") func = "unMute";
                else if (action === "volume") { func = "setVolume"; postArgs = [value !== undefined ? value : 75]; }
                else if (action === "skip") { func = "seekTo"; postArgs = [30, true]; }
            ...
            callback({ result: `Media control command '${action}' completed.` });
            break;
          }
          case "browserScroll": {
            ...
            callback({ result: `Scrolled browser view ${direction}.` });
            break;
          }
          case "browserType": {
            ...
            callback({ result: `Typed text into input field.` });
            break;
          }
          case "browserGoBack": {
            handleBack();
            callback({ result: "Navigated back to previous page." });
            break;
          }
          ...
          case "browserReadContent": {
            const targetUrl = args.url || (activeTab ? activeTab.url : "");
            if (!targetUrl || targetUrl === "about:blank") {
              callback({ result: { title: "Blank Page", headings: [], paragraphs: [], links: [] } });
              break;
            }

            if (targetUrl.includes("youtube.com/results") && ytSearchResults.length > 0) {
              const videoSummary = ytSearchResults.map((v, i) => `${i + 1}. "${v.title}" by ${v.channelTitle} (ID: ${v.videoId})`).join("\n");
              callback({
                result: {
                  url: targetUrl,
                  title: "YouTube Search Results",
                  headings: ["YouTube Video Results:"],
                  paragraphs: [videoSummary],
                  links: ytSearchResults.map(v => `https://youtube.com/watch?v=${v.videoId}`)
                }
              });
              break;
            }
            ...
            try {
              ...
              if (proxyRes.ok) {
                ...
                callback({ result: proxyData });
              } else {
                callback({ result: { url: targetUrl, title: activeTab?.title || "Webpage", message: "Content loaded." } });
              }
            } catch (err: any) {
              callback({ result: { url: targetUrl, error: err.message } });
            }
            break;
          }
          default: {
            callback({ error: `Command ${type} not recognized.` });
          }
        }
      } catch (err: any) {
        callback({ error: `Browser action error: ${err.message}` });
      }
    };

    runVoiceAutomation();
  }, [actionTrigger, activeTabId, isLocalConnected, ytSearchResults]);
  ...
  const getCleanTitleFromUrl = (urlStr: string): string => {
    ...
    try {
      ...
      if (parsed.hostname.includes("google.com")) {
        if (parsed.pathname.includes("maps") || parsed.hostname.includes("maps")) {
          let q = "";
          if (parsed.pathname.includes("/search/")) {
            q = decodeURIComponent(parsed.pathname.split("/search/")[1] || "");
          }
  ...
  const navigateToUrl = (targetUrl: string) => {
    ...
    if (!finalUrl.startsWith("http://") && !finalUrl.startsWith("https://")) {
      ...
      if (lower === "youtube" || lower === "youtube.com") finalUrl = "https://youtube.com";
      else if (lower.startsWith("youtube ")) {
        const ytQ = finalUrl.replace(/^youtube\s+/gi, "").trim();
        finalUrl = `https://youtube.com/results?search_query=${encodeURIComponent(ytQ)}`;
      }
      else if (lower === "google" || lower === "google.com") finalUrl = "https://google.com";
      else if (lower === "maps" || lower === "google maps" || lower === "maps.google.com" || lower === "google.com/maps") finalUrl = "https://www.google.com/maps";
      else if (lower.startsWith("maps ") || lower.startsWith("google maps ")) {
        const mapQ = finalUrl.replace(/^(google\s+)?maps\s+(for\s+)?/gi, "").trim();
        finalUrl = mapQ ? `https://www.google.com/maps/search/${encodeURIComponent(mapQ)}` : "https://www.google.com/maps";
      }
      else if (lower === "wikipedia" || lower === "wikipedia.org") finalUrl = "https://wikipedia.org";
      else if (lower === "twitter" || lower === "x" || lower === "x.com") finalUrl = "https://x.com";
      else if (lower === "instagram" || lower === "instagram.com") finalUrl = "https://instagram.com";
      else if (lower === "github" || lower === "github.com") finalUrl = "https://github.com";
      else if (lower === "reddit" || lower === "reddit.com") finalUrl = "https://reddit.com";
      else if (lower === "amazon" || lower === "amazon.com") finalUrl = "https://amazon.com";
      else {
  ...
  const handleNewTab = (initialUrlStr: string = "about:blank") => {
    if (initialUrlStr !== "about:blank") {
      ...
      if (existingTab) {
        ...
        if (initialUrlStr !== existingTab.url) {
          navigateToUrl(initialUrlStr);
        }
        return;
      }
    }
    ...
    const newTab: Tab = {
      ...
      currentIndex: 0,
      isLoading: initialUrlStr !== "about:blank" && !restrictions.restricted,
      openedExternally: restrictions.restricted
    };
    setTabs(prev => [...prev, newTab]);
    setActiveTabId(newTab.id);
    setInputValue(initialUrlStr === "about:blank" ? "" : initialUrlStr);
  };
  ...
  const getRenderUrl = (urlStr: string) => {
    ...
    // Google Maps embed renderer using output=embed parameter
    if (urlStr.includes("google.com/maps") || urlStr.includes("maps.google.com") || urlStr.toLowerCase().startsWith("maps")) {
      let searchQuery = "";
      try {
        const parsed = new URL(urlStr);
        if (parsed.pathname.includes("/search/")) {
          searchQuery = decodeURIComponent(parsed.pathname.split("/search/")[1] || "");
        } else if (parsed.searchParams.get("q")) {
          searchQuery = parsed.searchParams.get("q") || "";
        }
      } catch {
        if (urlStr.toLowerCase().startsWith("maps ")) {
          searchQuery = urlStr.substring(5).trim();
        }
      }
      return `https://maps.google.com/maps?q=${encodeURIComponent(searchQuery || "world")}&output=embed`;
    }
    ...
    if (activeTab) {
      ...
      try {
        ...
        if (iframe && iframe.contentDocument) {
          ...
          if (bodyTxt.includes("Luci Web Proxy Error") || bodyTxt.includes("Proxy Error") || bodyTxt.includes("Failed loading remote website")) {
              ...
              <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center space-y-8 select-none text-center scrollbar-none">
                ...
                <div className="p-3 border border-white/5 bg-slate-950/50 rounded-2xl max-w-md w-full">
                  <p className="text-[9px] font-mono text-slate-500 leading-normal uppercase">
                    🌸 COMPANION TIP: Voice command Luci to trigger any automation: <br />
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-sm mx-auto font-medium">
                      Luci was unable to load <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span> securely. This website may be offline, spelled incorrectly, or blocking automated server access.
                    </p>
                  </div>
                  ...
                  {!isLocalConnected ? (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 pr-1 select-text">
                      <div className="p-3.5 rounded-xl border border-indigo-500/10 bg-indigo-505/10 text-[11px] font-sans leading-normal text-indigo-300">
                        ...
                        <span>If you prefer to command a real, headed Chrome browser on your desktop computer, easily launch Luci's local client script!</span>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 pr-1 select-text scrollbar-thin">
                      {localLogs.length === 0 ? (
                        <div className="text-slate-600 italic">No operations recorded yet. Speaking with Luci will generate real terminal logs.</div>
                      ) : (
                        localLogs.map((log) => (
                          <div
                            key={log.id}
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-[10px] tracking-wide text-slate-400 leading-normal max-h-48 overflow-y-auto select-text scrollbar-thin">
                ...
                <div className="md:col-span-4 flex items-center justify-between border-b border-white/5 pb-1.5 mb-1 text-[11px] font-extrabold uppercase text-amber-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Active Luci Browser Diagnostics console</span>
                  </div>
